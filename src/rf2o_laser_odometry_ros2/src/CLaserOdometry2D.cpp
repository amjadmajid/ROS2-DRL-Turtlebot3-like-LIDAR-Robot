...
namespace rf2o
{
  ...
  CLaserOdometry2D::CLaserOdometry2D() : Node("CLaserOdometry2D"),
                                         verbose(false),
  ...
  void CLaserOdometry2D::calculaterangeDerivativesSurface()
  {
    ...
    for (unsigned int u = 1; u < cols_i - 1; u++)
      dtita(u) = (rtita(u - 1) * (range_inter[image_level](u + 1) -
                  ...
                  rtita(u) * (range_inter[image_level](u) -
                              range_inter[image_level](u - 1))) /
                 (rtita(u) + rtita(u - 1));
  ...
  {
    A.resize(num_valid_range, 3);
    Aw.resize(num_valid_range, 3);
    B.resize(num_valid_range, 1);
    Bw.resize(num_valid_range, 1);
    unsigned int cont = 0;
  ...
  void CLaserOdometry2D::PoseUpdate()
  {
    ...
    RCLCPP_INFO(get_logger(), "BASEodom = [%7.3f %7.3f %7.3f]",
