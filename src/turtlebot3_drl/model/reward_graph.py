...
names = ['DDPG 4 (1e-3)', 'DDPG 5 (3e-4)', 'DDPG 6 (1e-4)']

TOP_EPISODES = 4

def main(args=sys.argv[1:]):
    EPISODES = int(args[0])
    PLOT_INTERVAL = int(args[1])
    models = args[2:]
    ...
    for model in models:
        ...
        # plt.plot(xaxis, average_rewards, label=names[j])
    ...
    plt.savefig(os.path.join("graphs/", 'reward_graph_' + dt_string + '__' + suffix + ".png"), format="png", bbox_inches="tight")
    plt.show()
