...
class SACagent(Node):
    def __init__(self, stage, agent, episode):
        ...
        self.agent = SAC(self.device, self.state_size, self.action_size, self.discount_factor, self.learning_rate, self.tau)
        ...
        models_directory = (os.path.dirname(os.path.realpath(__file__))).replace(
            'install/turtlebot3_drl/lib/python3.8/site-packages/turtlebot3_drl/sac_agent',
...
def main(args=sys.argv[1:]):
    ...
    sac_agent = SACagent(args[0], args[1], args[2])
